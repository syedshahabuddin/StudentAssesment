...
include("header.php");
 ...
 /*$wordChunks = explode("-",isset( $_POST["dob"]));
for($i = 0; $i < count($wordChunks); $i++)
{
$name[$i] = $wordChunks[$i] ;
}*/
if(strlen(isset($name[1])) == 1)
{
$name[1] = "0". $name[1];
}

$dbda = isset($name[2]) . "-" . isset($name[0]). "-". isset($name[1]);
if(isset($_POST["button"]))
{
$sql="INSERT INTO studentdetails (studid, studfname, studlname, fathername, gender, address, contactno,courseid,semester)
VALUES
('$_POST[studid]','$_POST[studfname]','$_POST[studlname]','$_POST[fname]','$_POST[gender]','$_POST[address]','$_POST[contact]','$_POST[course]','$_POST[semester]')";

if (!mysql_query($sql,$con))
  {
  echo "The student ID alreadt exists please provide the valid ID with unique number";
  }
  else
  {
